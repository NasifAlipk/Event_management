...
export default function AddCoupon() {
  ...
  const validate = () => {
    if (!/^[A-Za-z0-9-]{3,40}$/.test(form.code)) {
      return "Coupon code must be 3-40 characters using letters, numbers, or hyphens.";
    }
    ...
    if (
      ...
      return "Amounts and usage limits cannot be negative.";
    }

    if (
      Number(form.usage_limit_per_user) > 0 &&
      Number(form.usage_limit_total) > 0 &&
      Number(form.usage_limit_per_user) >
        Number(form.usage_limit_total)
    ) {
      return "Per-user usage cannot exceed the total limit.";
    }
  ...
  const create = async () => {
    ...
    try {
      ...
      navigate("/coupons", {
        ...
        state: {
          message:
            "Coupon submitted and is pending admin approval.",
        },
      });
    } catch (requestError) {
    ...
    <ExploreLayout>
      <main className="min-h-screen bg-[#0d1418] px-4 pb-16 pt-24 text-white sm:px-8">
        <div className="mx-auto max-w-5xl">
          ...
          >
            <section className="rounded-2xl border border-white/10 bg-[#171e25] p-5">
              <h2 className="border-l-4 border-[#00ff85] pl-3 font-semibold">
                Basic Information
              </h2>

              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                {fields.slice(0, 2).map(([name, label]) => (
                  <label
                    key={name}
                  ...
                  >
                    ...
                    <input
                      ...
                      value={form[name]}
                      onChange={change}
                      className="mt-1.5 w-full rounded-lg border border-white/10 bg-[#111820] px-3 py-2.5 text-white"
                    />
                  </label>
                ))}
                ...
                <label className="text-sm text-slate-300 sm:col-span-2">
                  ...
                  <textarea
                    ...
                    maxLength={300}
                    rows="3"
                    className="mt-1.5 w-full rounded-lg border border-white/10 bg-[#111820] p-3 text-white"
                  />
                </label>
              </div>
            </section>
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                type="submit"
                disabled={saving}
                className="rounded-lg bg-[#00ff85] px-5 py-2.5 text-sm font-semibold text-black"
              >
                Create coupon
              </button>
            </div>
        ...
        {confirm && (
          <ConfirmModal
            title="Submit Coupon?"
            message="This coupon will be sent to the admin for approval."
